    ...
    this.twitterModel.findOne({profile: 'default'}).then(function(twitterParameters){
      if(twitterParameters) {
        ...
        if(this.follow && this.track && this.feedChannelID) {  
          if(this.discordClient.guilds.cache.get(process.env.HOME_GUILD)) {
            this.feedChannel = this.discordClient.guilds.cache.get(process.env.HOME_GUILD).channels.cache.get(this.feedChannelID);
          } else {
            return;
          }
